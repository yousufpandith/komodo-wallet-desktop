...
struct tests_context : public antara::gaming::world::app
{
  ...
  public:
    ...
    tests_context([[maybe_unused]] char** argv)
    {
        ...
        if (not wallet_manager.get_wallets().contains("uidex-wallet_tests"))
        {
            wallet_manager.create(
                test_password != nullptr ? test_password : "fakepasswordtemporary", test_seed != nullptr ? test_seed : "fake seed", "uidex-wallet_tests");
        }
        else
        {
            SPDLOG_INFO("uidex-wallet_tests already exists - skipping");
        }
        wallet_manager.login(test_password != nullptr ? test_password : "fakepasswordtemporary", "uidex-wallet_tests");
