...
WORKDIR /build/uidex-wallet-desktop
COPY . /build/uidex-wallet-desktop
...
RUN git config --global --add safe.directory /build/uidex-wallet-desktop
RUN cd /build/uidex-wallet-desktop && ./ci_tools_atomic_dex/ci_scripts/linux_script_docker.sh
...
RUN /build/uidex-wallet-desktop/ci_tools_atomic_dex/ci_scripts/choosenim.sh -y && \
    export PATH=/root/.nimble/bin:$PATH && \
    chmod +x /root/.choosenim/toolchains/nim-1.6.2/bin/*
ENV PATH=/root/.nimble/bin:$PATH
    
RUN cd /build/uidex-wallet-desktop/ci_tools_atomic_dex/vcpkg-repo && ./bootstrap-vcpkg.sh
